...
const corsOptions = {
    ...
    credentials: true, // Include this line if needed
  };
...
MongoClient.connect("mongodb+srv://maher:maher@cluster0.ctrzcqa.mongodb.net/")
  ...
  // Fetch and broadcast the updated document count to connected clients for the "names" collection
    ...
    app.get("/getDocumentCount", (req, res) => {
        // Fetch counts for both collections
        const fetchCounts = async () => {
          try {
            const superheroesCount = await collection.countDocuments({});
            const namesCount = await anotherCollection.countDocuments({});
      
            return { superheroesCount, namesCount };
          } catch (error) {
            console.error(error);
        ...
        fetchCounts()
          .then(({ superheroesCount, namesCount }) => {
            console.log("Sending response:", { superheroesCount, namesCount });
            res.status(200).json({"superheroesCount": superheroesCount, "namesCount": namesCount });
          })
          .catch((error) => {
            console.error(error);
